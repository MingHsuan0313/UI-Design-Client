import { StyleLibrary } from 'src/app/shared/styleLibrary';
        ...
        editor.graph.addMouseListener(
            {
                ...
                mouseUp: function (sender, evt) {
                    ...
                    if (selectedCell != undefined) {
                        if (selectedCell.type == "tag") {
                            if (selectedCell.state == false) {
                                let select = StyleLibrary[0]["tagclicked"];
                                selectedCell.setStyle(StyleEditorService.convertJsonObjectToStyleDescription(select));
                                selectedCell.state = true;
                            }
                            else {
                                let deselect = StyleLibrary[0]["tag"];
                                selectedCell.setStyle(StyleEditorService.convertJsonObjectToStyleDescription(deselect));
                                selectedCell.state = false;
                            }
                        }
                        else if (selectedCell.type == "tree") {
                            if (selectedCell.icon && selectedCell.state) {
                                const iconStyle = StyleLibrary[0]["treeIconRight"];
                                selectedCell.setStyle(StyleEditorService.convertJsonObjectToStyleDescription(iconStyle));
                                for (let i = 0; i < selectedCell.visibleChild.length; i++) {
                                    if (selectedCell.visibleChild[i].iconNode != undefined)
                                        selectedCell.visibleChild[i].iconNode.setVisible(false);
                                }
                                selectedCell.state = false;
                            }
                            else if (selectedCell.icon && !selectedCell.state) {
                                const iconStyle = StyleLibrary[0]["treeIconDown"];
                                selectedCell.setStyle(StyleEditorService.convertJsonObjectToStyleDescription(iconStyle));
                                for (let i = 0; i < selectedCell.visibleChild.length; i++) {
                                    if (selectedCell.visibleChild[i].iconNode != undefined)
                                        selectedCell.visibleChild[i].iconNode.setVisible(true);
                                }
                                selectedCell.state = true;
                            }
                        }
