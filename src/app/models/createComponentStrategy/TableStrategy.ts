...
export class TableStrategy implements ICreateComponentStrategy {
  ...
  createComponent(graphStorage: GraphStorage, component, parent) {
    ...
    const headerList = component.headers.split(" ");
    const colNumber = headerList.length;
    const rows = component.rows.split(" ");
    const rowNumber = 1;
    ...
    for (let i = 0; i < colNumber; i++) {
      ...
      const tableHeaderVertexStorage = graphStorage.insertVertex(tableBoxVertexStorage.getVertex(), component.id + "header", headerList[i], tableHeaderVertexGeometry, styleStorage, component);
    ...
    for (let i = 0; i < colNumber; i++) {
      ...
      const tableDataVertexStorage = graphStorage.insertVertex(tableBoxVertexStorage.getVertex(),
        component.id + "header", rows[i], tableDataVertexGeometry, styleStorage, component);
